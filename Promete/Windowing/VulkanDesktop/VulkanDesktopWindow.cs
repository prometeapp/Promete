...
public class VulkanDesktopWindow : IWindow
{
		...
		private unsafe Image TakeScreenshotAsImage()
		{
			...
			{
				// TODO: Vulkanの方法でフレームバッファからピクセルデータを取得する
		...
		private void OnLoad()
		{
			_driver = new VulkanDriver(window);
			_RawInputContext = window.CreateInput();
			textureFactory = new VulkanTextureFactory(app);
			UpdateWindowSize();
