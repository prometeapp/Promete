...
4. **`TextureFactory` の参照先を変更する**
   ...
   <Tabs>
     <TabItem label="v1">
       ```csharp var texture = Window.TextureFactory.Load("image.png"); ```
     </TabItem>
     <TabItem label="v2">
       ```csharp var texture = App.TextureFactory.Load("image.png"); ```
     </TabItem>
   </Tabs>
...
```
   ...
   <Tabs>
     <TabItem label="v1">
       ```csharp float angle = player.Location.Angle(mouse.Position);
       sprite.Angle = angle; ```
     </TabItem>
     <TabItem label="v2">
       ```csharp Angle angle = player.Location.Angle(mouse.Position);
       sprite.Angle = angle; ```
     </TabItem>
   </Tabs>
...
7. **ビルドエラーと警告を確認する**
   ...
   <Aside type="tip">
     `[Obsolete]`
     警告が出ていても、動作自体はします。焦らず、警告が0になるまで順番に対応してください。
   </Aside>
