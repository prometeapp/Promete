...
v2 では `ShaderProgram` と `Material` を使って、個々のノードにカスタムシェーダーを適用できます。

:::note[シェーダー言語はバックエンド依存]
使用できるシェーダー言語はバックエンドによって異なります。現在提供されている **OpenGLDesktop バックエンドでは GLSL** が使用できます。将来的に他のバックエンド（Vulkan、Metal など）がサポートされた場合、そのバックエンド固有のシェーダー言語が必要になります。
:::
...
- OpenGLDesktop バックエンドでは GLSL 3.30 以上が使用できます
- uniform 変数の型として `float`・`int`・`Vector2`・`Vector3`・`Vector4`・`Matrix4x4` などが使用できます
- `uMvp`（モデル・ビュー・プロジェクション行列）と `uTexture`（テクスチャサンプラー）は、デフォルトシェーダーでも使用されている組み込みの uniform です
